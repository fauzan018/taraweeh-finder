...
export default function AddMasjid() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setLoading(true);
    await supabase.from("approved_mosques").insert([{ ...form, latitude: Number(form.latitude), longitude: Number(form.longitude), upvotes: 0, views: 0, approved_at: new Date().toISOString() }]);
    setLoading(false);
    setSuccess(true);
    setForm({
      name: "",
  ...
  return (
    <main className="min-h-screen bg-[var(--background)] p-4 flex flex-col items-center">
      <h1 className="text-2xl font-bold text-white mb-4">Add New Masjid</h1>
      <form onSubmit={handleSubmit} className="bg-[var(--card)] rounded-xl p-6 w-full max-w-md flex flex-col gap-3 shadow-xl">
        ...
        <Button type="submit" className="bg-[var(--primary)] text-black rounded-xl mt-2" disabled={loading}>{loading ? "Adding..." : "Add Masjid"}</Button>
        {success && <div className="text-green-400 mt-2">Masjid added successfully!</div>}
      </form>
    </main>
  );
}
